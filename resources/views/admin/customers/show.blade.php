            ...
            <div class="col-md-4">
                    ...
                    <div class="card-body box-profile">
                        ...
                        <ul class="list-group list-group-unbordered mb-3">
                            <li class="list-group-item">
                                <b>رقم الهاتف</b> <a
                                    class="float-end text-decoration-none">{{ $customer->phone ?? '—' }}</a>
                            </li>
                            <li class="list-group-item">
                                <b>واتساب</b> <a class="float-end text-decoration-none text-success"
                                    href="https://wa.me/{{ $customer->phone_whatsapp }}" target="_blank">
                                    {{ $customer->phone_whatsapp ?? '—' }} <i class="bi bi-whatsapp"></i>
                                </a>
                            </li>
                            <li class="list-group-item">
                                <b>العنوان</b> <span class="float-end small text-muted text-wrap text-end"
                                    style="max-width: 60%;">
                                    {{ implode(' - ', array_filter([$customer->area, $customer->street, "م: " . $customer->house_number])) ?: 'غير مسجل' }}
                                </span>
                            </li>
                            <li class="list-group-item">
                                <b>العملة المفضلة</b> <a class="float-end text-decoration-none"><span
                                        class="badge bg-secondary">{{ $customer->preferred_currency }}</span></a>
                            </li>
                        </ul>
                    </div>
                </div>
                ...
                <div class="card {{ $customer->balance->amount >= 0 ? 'card-success' : 'card-danger' }} card-outline">
                    ...
                    <div class="card-body text-center">
                        <h1
                            class="display-5 font-weight-bold {{ $customer->balance->amount >= 0 ? 'text-success' : 'text-danger' }}">
                            {{ number_format($customer->balance->amount, 2) }}
                            <small>{{ $customer->balance->currency }}</small>
                        </h1>
                ...
                <div class="card">
                    <div class="card-header p-2">
                        <ul class="nav nav-pills">
                            <li class="nav-item"><a class="nav-link active" href="#orders" data-bs-toggle="tab">الطلبات
                                    (الواتير)</a></li>
                            <li class="nav-item"><a class="nav-link" href="#payments" data-bs-toggle="tab">المدفوعات</a>
                            </li>
                        </ul>
                    </div>
                    <div class="card-body">
                                        ...
                                        <tbody>
                                            @forelse($customer->orders as $order)
                                                <tr>
                                                    ...
                                                    <td class="fw-bold">{{ number_format($order->total, 2) }}
                                                        {{ $order->currency }}</td>
                                                    <td>
                                            ...
                                            @empty
                                                <tr>
                                                    <td colspan="5" class="text-center text-muted">لا توجد طلبات سابقة</td>
                                                </tr>
                                            @endforelse
                                        </tbody>
                                    </table>
                                            ...
                                            @forelse($customer->payments as $payment)
                                                <tr>
                                                    ...
                                                    <td class="text-success fw-bold">+ {{ number_format($payment->amount, 2) }}
                                                        {{ $payment->currency }}</td>
                                                    <td><span class="badge bg-secondary">{{ $payment->payment_method }}</span>
                                                    </td>
                                                    <td>{{ $payment->note ?? '—' }}</td>
                                                </tr>
                                            @empty
