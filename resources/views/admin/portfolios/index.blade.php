            ...
            <table class="table table-striped projects">
                ...
                <tbody>
                    @foreach($portfolios as $portfolio)
                        <tr onclick="window.location='{{ route('admin.portfolios.edit', $portfolio->id) }}';"
                                    style="cursor: pointer;">
                                    <td>{{ $loop->iteration }}</td>
                                    <td>
                                        @if($portfolio->image)
                                            <img src="{{ asset($portfolio->image) }}" alt="Portfolio" class="img-fluid"
                                    ...
                                    <td class="project-actions text-right">
                                        <form action="{{ route('admin.portfolios.destroy', $portfolio->id) }}" method="POST"
                                            ...
                                            <button type="submit" class="btn btn-danger btn-sm"
                                                onclick="event.stopPropagation(); return confirm('هل أنت متأكد؟')">
